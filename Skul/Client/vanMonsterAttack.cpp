...
namespace van
{
	...
	{
		// MonsterAttack 클래스를 소유한 객체의 주소 저장
		Monster* monster = (Monster*)GetOwner();
		// MonsterAttack 클래스를 소유하고있는 객체의 상태
		Monster::MonsterState ownerState = (Monster::MonsterState)GetOwnerState();

		// 피격된 대상의 주소저장
		GameObject* obj = _other->GetOwner();	
		// 피격된 대상이 Player 인지 확인
		Player* playr = dynamic_cast<Player*>(obj);
		...
		if (ownerState == Monster::MonsterState::Attack 
			&& mbAttackReadyFlag == true)
		{
